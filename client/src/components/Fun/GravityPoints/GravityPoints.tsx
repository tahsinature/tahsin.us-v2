...
import classes from './GravityPoints.module.scss';

const GravityPoints = () => {
  const canvContainer = createRef();

  useEffect(() => {
    let height = _.get(canvContainer.current, 'offsetHeight', 0);
    let width = _.get(canvContainer.current, 'offsetWidth', 0);
    ...
    (function () {
      ...
      function mouseDown(e) {
        ...
        gravities.push(
          ...
          new GravityPoint(e.offsetX, e.offsetY, G_POINT_RADIUS, {
            particles: particles,
            gravities: gravities,
          }),
        );
  ...
  return (
    <div ref={canvContainer} className={classes.GravityPoints}>
      <canvas id="c" className={classes.Canvas}></canvas>
    </div>
  );
};

export default memo(GravityPoints, () => true);
